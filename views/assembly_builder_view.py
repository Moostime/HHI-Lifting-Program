...
class AssemblyBuilderWindow(ctk.CTkToplevel):
    def __init__(self, master, **kwargs):
        super().__init__(master, **kwargs)
        self.title("Wire Rope Assembly Builder")
        ...
        self.action_frame.grid(row=1, column=1, columnspan=2, padx=10, pady=10, sticky="ew")
        ...
        self.add_to_order_button = ctk.CTkButton(self.action_frame, text="Add to Order", command=self.add_to_order, state="disabled")
        self.add_to_order_button.grid(row=0, column=1, padx=5, sticky="ew")
    ...
    def _get_swage_factor(self, diameter_str):
        diameter_float = self._fraction_to_float(diameter_str)
        if diameter_float <= 1.0: return 0.95
        elif 1.125 <= diameter_float <= 2.0: return 0.925
        elif diameter_float >= 2.125: return 0.90
        return 1.0
    ...
    def generate_bom(self):
        ...
        # Get WLL for a SINGLE leg to use as the base for all calculations
        single_leg_loads = get_working_loads(diameter) or {}
        single_leg_vertical_wll = single_leg_loads.get('vertical', 0)
        
        wll_text = ""
        if assy_legs == 1:
            self.final_loads = single_leg_loads
            wll_text = f"WLL: Vertical: {self.final_loads.get('vertical', 0):.2f} | Choker: {self.final_loads.get('choker', 0):.2f} | Basket: {self.final_loads.get('basket', 0):.2f}"
        else:
            angle_factors = get_sling_angle_factors(assy_legs) or {}
            wll_at_60 = single_leg_vertical_wll * float(angle_factors.get('factor_60_deg', 0))
            wll_at_45 = single_leg_vertical_wll * float(angle_factors.get('factor_45_deg', 0))
            wll_at_30 = single_leg_vertical_wll * float(angle_factors.get('factor_30_deg', 0))
            self.final_loads = {'60_deg': wll_at_60, '45_deg': wll_at_45, '30_deg': wll_at_30}
            wll_text = f"WLL: 60°: {wll_at_60:.2f} | 45°: {wll_at_45:.2f} | 30°: {wll_at_30:.2f}"
        ...
        if self.main_assembly_item:
            ...
            test_load_value = single_leg_vertical_wll * 2
            ctk.CTkLabel(self.col3_frame, text="Test / Proof Load:").grid(row=3, column=0, padx=10, pady=5, sticky="w")
            self.test_load_entry = ctk.CTkEntry(self.col3_frame)
            self.test_load_entry.grid(row=3, column=1, padx=10, pady=5, sticky="ew")
            self.test_load_entry.insert(0, f"{test_load_value:.2f}")
        ...
        self.add_to_order_button.configure(state="normal")
    ...
    def add_to_order(self):
        if self.main_assembly_item:
            ...
            assy_legs = self.recipe[0].get('legs') if self.recipe else 1
            if assy_legs == 1:
                self.main_assembly_item['rated_load'] = f"Vertical: {self.final_loads.get('vertical', 0):.2f}"
            else:
                self.main_assembly_item['rated_load'] = f"60°: {self.final_loads.get('60_deg', 0):.2f}"
                
            self.main_assembly_item['test_load'] = self.test_load_entry.get()
            self.main_assembly_item['is_main_assembly'] = True
            items_for_order = [self.main_assembly_item]
        else:
            items_for_order = []
